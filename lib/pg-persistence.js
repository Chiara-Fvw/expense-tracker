...
module.exports = class PgPersistence {
  ...
  async existCategory(title) {
    const MATCH_TITLE = "SELECT FROM categories WHERE title = $1";

    let result = await dbQuery(MATCH_TITLE, title);
    return result.rowCount > 0;
  }

  //Authentication checker
  async authenticate(username, password) {
    const AUTHENTICATE = "SELECT null FROM users WHERE username = $1 AND password = $2";
